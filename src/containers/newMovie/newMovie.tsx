...
export class NewMovie extends React.Component<{}, INewMovieState> {
    ...
    addMovie = (event) => {
        ...
        this.setState({ movieName: '', addedPlanetNames: [], planetName: '', planets: [] })
    }

    // validate = name => {
    //     let validationError = this.state.formError;
    //     let valid = this.state.movieValid;
                    ...
                    <div className="add-movie_container">
                        <Form onSubmit={this.addMovie}>
                            <FormGroup>
                                ...
                                <Input type="text" placeholder="Please enter title of the movie" name="name" value={this.state.movieName} onChange={this.movieNameHandler} />
                            ...
                            {this.state.addedPlanetNames !== undefined && this.state.addedPlanetNames.length > 0 ?
                                (this.state.addedPlanetNames.map((element, index) => (
                                    <span className="add-movie_planet-name" key={index}>{element}<button className="close" onClick={e => this.removePlanetNameHandler(e, element)}/></span>
                                ))) : null
                            }
                            <FormGroup>
                                <Label>Add Planet</Label>
                                <Input type="text" placeholder="Search for the planet in database" value={this.state.planetName} onChange={this.planetsHandler} onClick={this.resultsOpenHandler}/>
                            ...
                            <Button type="submit" color="info">Add Movie</Button>
                        </Form>
                    </div>
                </div>
